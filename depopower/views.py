...
def main(request):
    ...
    for cellrecordjan in cell_records_jan:
        norm = TechWork.objects.values('norms_in_days').filter(model_name=cellrecordjan.locomotive,
                                                               to_name=cellrecordjan.techwork)
    ...
    percentage = (
                         percentage_jan + percentage_feb + percentage_mar + percentage_apr + percentage_may + percentage_jun + percentage_jul + percentage_aug + percentage_sep + percentage_oct + percentage_nov + percentage_dec) / 12
...
def edit(request):
    ...
    percentages = [{'jan': percentage_jan}]

    return render(request, 'depopower/edit.html', {'loco_data': loco_data, 'month_data': month_data,
                                                   'techwork_data': techwork_data,
                                                   'available_techworks': available_techworks,
                                                   'percentage': percentage,
...
def submit(request):
    ...
    if request.method == "POST":
        ...
        cellrecord.locomotive = Locomotive.objects.get(model_name=request.POST.get("machine_name"))
        cellrecord.techwork = AvailableTechwork.objects.get(to_name=request.POST.get("techwork_name"))
        cellrecord.month = MonthWorkDay.objects.get(month=request.POST.get("month_name"))
        try:
            ids = CellRecord.objects.get(locomotive=cellrecord.locomotive, techwork=cellrecord.techwork, month=cellrecord.month)
        except CellRecord.DoesNotExist:
            ids = None
        if ids:  # if some items are found in the database
            ids.volume = ids.volume + 1
            ids.save()
        else:
            cellrecord.save()
